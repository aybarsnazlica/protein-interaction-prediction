...
def write_normalized_contacts(contacts_result, norm_out_file):
    ...
    # df_cont["res1"] = df_cont["index"].str.split("_", expand=True)[0] + "_" + \
    #                   df_cont["index"].str.split("_", expand=True)[1]
    # df_cont["res2"] = df_cont["index"].str.split("_", expand=True)[2] + "_" + \
    #                   df_cont["index"].str.split("_", expand=True)[3]
...
def run_commands(contacts_inputs: list):
    ...
    # contacts_result = calc_contact_ratio(inputs)
    # norm_out_file = Path(f"{protein_complex.model_id}-norm.tsv")
...
def run_contact_calc_piper():
    ...
    # run_contact_piper_native(contact_output_path)
