...
PROJECT_DIR = Path("/u21/aybars/multiple-models/").resolve()
ATOM_NAMES_PATH = PROJECT_DIR / "work" / "atom_names"
SVD_MODEL_PATH = PROJECT_DIR / "models" / "svd_model_500"
# ML_MODEL_PATH = PROJECT_DIR / "models" / "xgboost" / "xgb_model_161.json"
...
def reduce_dimensions(norm_file_path):
    ...
    # nobs = 1  single observation
    # dim = 5967  dimensions svd model is trained with
    # reduced_matrix = np.zeros((nobs, dim), dtype=float)
    ...
    # full_matrix = parse_norm_file(namedict, pairindex, (nat * nat), norm_file_path)
    # np.savetxt("/u21/aybars/scratch/6l6gAB_full_matrix.csv", full_matrix.reshape(167, 167), delimiter=",")
    
    return reduced_mat

# def calculate_a_score(reduced_mat):
#     xgb_model = xgb.Booster()
#     xgb_model.load_model(ML_MODEL_PATH)
#     a_score = xgb_model.predict(xgb.DMatrix(reduced_mat))[0]
#     return a_score
...
#     model_id, feat_matrix, a_score_value, irmsd = outputs

#     with open(Path(WORK_DIR) / "report.tsv", "w") as out:
#         if args.debug:
#             # big matrix
#             # out.write("\t".join([model_id, f"{irmsd:.3f}"]))
#             # for n in feat_matrix:
#             #     out.write(f"\t{n:.5f}")
#             # out.write("\n")

#             # reduced matrix
#             out.write("\t".join([model_id, f"{a_score_value:.3f}", f"{irmsd:.3f}"]))
#             for n in feat_matrix[0][: (len(feat_matrix[0]) + 1)]:
#                 out.write(f"\t{n:.5f}")
#             out.write("\n")
#         else:
#             out.write(
#                 "\t".join([model_id, f"{a_score_value:.3f}", f"{irmsd:.3f}"]) + "\n"
#             )
